...
class ClassroomApp {
    ...
    initEventListeners() {
        // Solo mantener event listeners para comentarios
    ...
    async loadPosts() {
        try {
            ...
            // this.showNotification("Error","Error al cargar publicaciones" ,"error");
    ...
    async addComment(postId, form) {
        ...
        try {
            ...
            input.value = "";
            this.showNotification("Exito", "Comentario agregado");
            await this.loadPosts();

        } catch (error) {
            console.error("Error adding comment:", error);
            this.showNotification("Error", "No se pudo agregar el comentario");
        } finally {
    ...
    handleError(error, context = "") {
        ...
        if (error.message.includes("Sesión expirada") || error.message.includes("401")) {
            this.showNotification("Error", "Tu sesión ha expirado. Por favor, inicia sesión nuevamente.", "error");
            setTimeout(() => {
    ...
    async openAIMaterial(materialId) {
        try {
            console.log("📖 Abriendo material IA:", materialId);
            ...
            const material = await this.makeRequest(`/api/classroom/${classroomId}/material/${materialId}`);
            if (!material || !material.content) {
                throw new Error("No se recibió material");
            }
            ...
            // Insertar el material completo en el modal
            const previewContainer = modal.querySelector(".material-preview");
            previewContainer.innerHTML = this.renderFullMaterial(material);
    ...
    renderFullMaterial(output) {
        ...
        <div class="preview-content">
          ...
          ${content.conceptos_clave && content.conceptos_clave.length > 0 ? `
            ...
            <ul>
              ${content.conceptos_clave.map(c => `<li>${c}</li>`).join('')}
            </ul>
          ` : ''}
          ...
          ${content.aplicaciones_practicas && content.aplicaciones_practicas.length > 0 ? `
            ...
            <ul>
              ${content.aplicaciones_practicas.map(a => `<li>${a}</li>`).join('')}
            </ul>
          ` : ''}
        </div>
      `;
        ...
        <div class="preview-content">
          ...
          <p>Total: ${content.preguntas ? content.preguntas.length : 0} preguntas</p>
          ${content.preguntas && content.preguntas.length > 0 ? `
            <ol>
              ${content.preguntas.map((q, qi) => `
                <li>
                  <p><strong>${qi + 1}.</strong> ${q.enunciado}</p>
                  <ul>
                    ${q.opciones.map((op, oi) =>
                    `<li>${String.fromCharCode(65 + oi)}. ${op}</li>`
                ).join('')}
                  </ul>
                  ${q.respuesta_correcta !== undefined ? `
      ...
      `;
          ...
          <div class="preview-content">
            ...
            ${content.nodos && content.nodos.length > 0 ? `
              <ul>
                ${content.nodos.map(n => `<li>${n}</li>`).join('')}
              </ul>
            ` : '<p>Mapa mental disponible.</p>'}
          </div>
        `;
                }
                return `
        <div class="preview-content">
          ...
          ${content.tarjetas && content.tarjetas.length > 0 ? `
            <ul>
              ${content.tarjetas.map((t, i) => `
                <li><strong>${i + 1}.</strong> ${t.pregunta} → ${t.respuesta}</li>
              `).join('')}
            </ul>
